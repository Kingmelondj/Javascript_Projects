...
function placeXOrO(squareNumber) {
    ...
    if (!selectedSquares.some(element => element.includes(squareNumber))) {
        ...
        //  Active player may only be "X" or "O" so, if not "X" it bust be "O".
        }else {
...
function checkWinConditions() {
    ...
    // and 9 squares are selected, the executes.
    else if (selectedSquares.length >= 9) {
        // This funcitons plays the tie game sound.
        audio('./media/tie.mp3');
...
function drawWinline(coordX1, coordY1, coordX2, coordY2) {
    ...
    const canvas = document.getElementById('win_lines');
    ...
    function animateLineDrawing() {
        ...
        // It wasa necessary for the 6, 4, 2 win condition.
        if (x1 <= x2 && y1 >= y2) {
            if (x < x2) { x += 10; }
            if (y > y2) { y -= 10; }
            if (x >= x2 && y <= y2) { cancelAnimationFrame(animationLoop);}
